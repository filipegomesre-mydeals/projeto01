...
const radioGroupVariants = cva(
    'aspect-square size-3 flex  items-center justify-center shrink-0 rounded-full ring-[1.5px] ring-gray-300 focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-40',
    {
        variants: {
            ...
            color: {
                default:
                    'data-[state=checked]:ring-black dark:data-[state=checked]:ring-white',
...
>(({ className, variant, color, ...props }, ref) => {
    return (
        <RadioGroupPrimitive.Item
            ...
            className={cn(
                '',
                radioGroupVariants({ variant, color, className }),
            )}
            {...props}
        >
...
>(({ className, ...props }, ref) => {
    return (
        <RadioGroupPrimitive.Item
            ref={ref}
            className={cn(
                'aspect-square size-3 rounded-full border-[1.5px] border-gray-300 focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-0 data-[state=checked]:bg-black dark:data-[state=checked]:bg-white',
            )}
            {...props}
        >
            <RadioGroupPrimitive.Indicator className="flex items-center justify-center">
                <IconCheckboxCheck
                    className={cn('size-1.5 text-white dark:text-black')}
                />
            </RadioGroupPrimitive.Indicator>
        </RadioGroupPrimitive.Item>
    )
